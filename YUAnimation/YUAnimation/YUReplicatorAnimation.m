...
+ (CALayer *)replicatorLayer_Circle{
    ...
    replicatorLayer.instanceDelay = 0.5; // 在短时间内的肤质延时
    replicatorLayer.instanceCount = 8; //拷贝图层的次数，包括其所有的子图层
...
+ (CALayer *)replicatorLayer_Wave{
    ...
    replicatorLayer.instanceDelay = 0.2; //在短时间内的复制延时
    replicatorLayer.instanceCount = 3;  //拷贝图层的次数，包括其所有的子图层
    replicatorLayer.instanceTransform = CATransform3DMakeTranslation(between*2+radius,0,0);//复制图层时产生的和上一个图层的位移
    [replicatorLayer addSublayer:shapeLayer];
...
//    CATransform3DRotate(CATransform3D t, CGFloat angle, CGFloat x, CGFloat y, CGFloat z)
    ...
    scale.repeatCount = HUGE;
//    淡入淡出(先慢 后慢 中间快) kCAMediaTimingFunctionEaseInEaseOut  Ease(慢)
